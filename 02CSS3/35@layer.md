...
- [@layer](#layer)
  ...
  - [嵌套的 @layer](#%E5%B5%8C%E5%A5%97%E7%9A%84-layer)
    - [嵌套 @layer 的优先级](#%E5%B5%8C%E5%A5%97-layer-%E7%9A%84%E4%BC%98%E5%85%88%E7%BA%A7)
...
使用级联层, 开发者可以更灵活地控制级联规则. 写在级联层 `A` 中的 `CSS` 规则与写在级联层 `B` 中的 `CSS` 规则完全没有关系, 究竟哪个规则生效? `只与`级联层有关系(非 `!important`). 因此, 当我们想要引入第三方样式文件又不想让第三方某些优先级过高的 `CSS` 规则覆盖原本的重名的规则, 就可以在级联层中引入.
...
```
2️⃣ 第二种, 创建一个具名的级联层, 但是没有任何 `CSS` 规则. 这种方式创建一个级联层或同时创建多个级联层. 创建多个级联层的同时, 他们的优先级顺序也确定了, 即 `layout` 的优先级比 `colors` 更高.
```css
@layer colors;
@layer colors, layout;
```
但是如果写成这样也很奇怪, 因为没有具体的 `CSS` 规则, 因此在声明之后可以重复再声明一次并添加 `CSS` 规则, 并且级联层的优先级不会发生变化
```css
...
如果我们直接追加没有声明过的的级联层呢? 比如下面代码中, 增加了一个不存在的外层 `outerDontKnow` 和不存在的内层 `inner`; 还在存在的外层中增加了不存在的内层 `innerDontKnow`, 所以最后有几个兔子呢?
```css
@layer outer.inner {
  .box3::before {
    content: '🐰'
...
## @layer 与 `!important`
使用了 `!important` 标志, 优先级将会发生重大改变. 先声明的级联层中的 `!important` 高于后声明的, 而且所有级联层中的 `!important` 高于非级联层中的 `!important`.
```css
@layer outer2 {
  .box5 {
    width: 100px;
...
```
根据上面的优先级规则, 可以看到元素背景色是 `pink`, 边框是 `red`.
![](../image/Snipaste_2023-02-02_09-15-58.png)

接下来就出现了另一个问题, 在刚开始的时候说了可以用级联层引入第三方样式, 如果第三方样式中没有 `!important`, 我们就可以轻易覆盖. 那如果第三方样式中有 `!important` 中呢, 那就是用上面例子的方法覆盖! 即优先级最低的级联层用来写 `!important` 专门覆盖第三方 `!important`.

## 浏览器兼容性
这么新的特性, `IE` 是肯定不支持. `Chrome 99` 及以后版本、`Firefox 97` 及以后版本和 `Safari 15.4` 及以后版本支持, 具体可见 [caniuse](https://caniuse.com/?search=%40layer).
