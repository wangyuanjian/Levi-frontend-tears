...
1. 添加路由(在导航守卫中添加路由)
    ...
    - 下面在路由导航中展示添加路由的例子.
      ...
      - ![](../image/Snipaste_2022-08-20_09-32-33.png)
    - 📕一个注意点: 当你添加完导航时还是 `/error` 还是不能导航到 `Error` 组件. 这是因为路由只是添加, 你只有切换到 `/bye` 再切换回 `/error` 才可以
2. 删除路由
    - 使用 `addRoute()` 的返回值删除路由
      - ```js
        const removeRoute = router.addRoute(routeRecord)
        removeRoute() // 删除路由如果存在的话
    - 使用 `router.removeRoute(name)` 按名称删除路由
      - ```js
        outer.removeRoute('about')
3. 查看现有路由
    - 使用 `router.hasRoute(name)` 判断是否存在对应路由
    - 使用 `router.getRoutes()` 获取全部路由
    - ```js
      function myAddRoute() {
        router.addRoute({
          path: '/error',
        ...
        console.log('has error route', router.hasRoute('error'))
        console.log('add---', router.getRoutes())
      }
    - ![](../image/Snipaste_2022-08-20_09-46-59)